...
def search_uniprot(request, query_text, query_size, form, title):
    ...
    for result in results[:query_size]:
        ...
        info.append([header, result])
    return render(request, 'zapytanie_uniprot.html', {'title': title, 'results': results[:query_size], 'info': info})

def zapytanie_uniprot(request):
    if request.method == 'POST':
        for ele in request.POST:
            if ele != 'csrfmiddlewaretoken':
                database_id = request.POST[ele]
                if request.user.is_authenticated:
                    post = Post(database_id = database_id, database_choice='Uniprot', title=ele, author=request.user)
                else:
                    post = Post(database_id = database_id, database_choice='Uniprot', title=ele)
                post.type = 'database'
                post = uniprot_info(database_id, post)
                post.save()
        return redirect('/post')

def uniprot_info(id, post):
    ...
    if request.method == 'POST':
        for ele in request.POST:
            if ele != 'csrfmiddlewaretoken':
                id = request.POST[ele]
                if request.user.is_authenticated:
                    post = Post(database_id=id, database_choice='PDB', title=ele, author=request.user)
                else:
                    post = Post(database_id=id, database_choice='PDB', title=ele)
                post.type = 'database'
